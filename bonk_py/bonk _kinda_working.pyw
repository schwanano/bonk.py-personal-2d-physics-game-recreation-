...
class Rect:
    ...
    def render(self, surf):
        self.surf = pygame.Surface((self.width * 8, self.height * 8), pygame.SRCALPHA)
        self.surf.fill(self.color)
        self.surf = pygame.transform.smoothscale(self.surf, (self.width, self.height))
        self.surf = pygame.transform.rotate(self.surf, math.degrees(self.facing))
        blit_loc_x = min(self.t_r.x, self.b_r.x, self.b_l.x, self.t_l.x)
        blit_loc_y = min(self.t_r.y, self.b_r.y, self.b_l.y, self.t_l.y)
        blit_location = pygame.math.Vector2(blit_loc_x, blit_loc_y)
        surf.blit(self.surf, blit_location)
        
        if self.bouncy:
            pygame.draw.aalines(
                surf, "black", True, [self.t_r, self.b_r, self.b_l, self.t_l]
                )
        elif self.death:
            pygame.draw.aalines(
                surf, "red", True, [self.t_r, self.b_r, self.b_l, self.t_l]
                )
        else:
            pygame.draw.aalines(
                surf, self.color, True, [self.t_r, self.b_r, self.b_l, self.t_l]
                )
